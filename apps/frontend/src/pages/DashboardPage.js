...
export const DashboardPage = () => {
    ...
    return (_jsxs("div", { className: "grain-overlay min-h-screen bg-background relative overflow-hidden", children: [_jsx("div", { className: "ambient-blob blob-cyan", style: { top: '-200px', left: '-100px' } }), _jsx("div", { className: "ambient-blob blob-blue", style: { bottom: '-150px', right: '-100px' } }), _jsx("div", { className: "ambient-blob blob-orange", style: { top: '40%', right: '20%' } }), _jsx(Header, { onCitySelect: handleCitySelect }), _jsxs("main", { className: "pt-16 px-4 pb-4 max-w-[1800px] mx-auto relative z-10", children: [_jsxs("div", { className: "flex items-center gap-3 py-3 flex-wrap", children: [_jsxs("span", { className: "text-xs text-muted-foreground font-body uppercase tracking-wider", children: [t('dashboard.layers'), ":"] }), _jsxs("button", { onClick: () => setShowFires(!showFires), className: `flex items-center gap-1.5 px-3 py-1.5 text-xs font-body rounded border transition-all ${showFires
                                    ? 'bg-accent/15 border-accent/40 text-accent'
                                    : 'bg-muted border-border text-muted-foreground hover:text-foreground'}`, children: [_jsx(Flame, { className: "w-3.5 h-3.5" }), t('dashboard.fires')] }), _jsxs("button", { onClick: () => setShowDeforestation(!showDeforestation), className: `flex items-center gap-1.5 px-3 py-1.5 text-xs font-body rounded border transition-all ${showDeforestation
                                    ? 'bg-green-500/15 border-green-500/40 text-green-400'
                                    : 'bg-muted border-border text-muted-foreground hover:text-foreground'}`, children: [_jsx(Trees, { className: "w-3.5 h-3.5" }), t('dashboard.deforestation')] }), _jsxs("button", { onClick: () => setShowStations(!showStations), className: `flex items-center gap-1.5 px-3 py-1.5 text-xs font-body rounded border transition-all ${showStations
                                    ? 'bg-blue-500/15 border-blue-500/40 text-blue-400'
                                    : 'bg-muted border-border text-muted-foreground hover:text-foreground'}`, children: [_jsx(Radio, { className: "w-3.5 h-3.5" }), t('dashboard.stations')] }), _jsx("div", { className: "ml-auto", children: _jsxs("button", { onClick: viewMode === 'compare' ? handleCloseCompare : handleEnterCompare, className: `flex items-center gap-1.5 px-3 py-1.5 text-xs font-body rounded border transition-all ${viewMode === 'compare'
                                        ? 'bg-primary/15 border-primary/40 text-primary'
                                        : 'bg-muted border-border text-muted-foreground hover:text-foreground'}`, children: [_jsx(GitCompare, { className: "w-3.5 h-3.5" }), viewMode === 'compare' ? t('dashboard.exitComparison') : t('dashboard.compareCities')] }) })] }), _jsxs("div", { className: "flex gap-4", children: [_jsx(BrazilMap, { selectedCityId: selectedCityId, showFires: showFires, showDeforestation: showDeforestation, showStations: showStations, fires: fires.map(f => ({ lat: f.lat, lng: f.lng, intensity: f.intensity, state: f.state })) }), _jsx("div", { className: "hidden lg:block", children: viewMode === 'compare' ? (_jsx(ComparisonPanel, { cityA: compareCityA, cityB: compareCityB, onChangeCityA: setCompareCityA, onChangeCityB: setCompareCityB, onClose: handleCloseCompare })) : viewMode === 'city' && selectedCityId ? (_jsx(CityDashboard, { cityId: selectedCityId, onClose: handleCloseCity })) : (_jsx(AQISidebar, {})) })] }), _jsxs("footer", { className: "mt-4 flex flex-col sm:flex-row items-center justify-between text-xs text-muted-foreground py-3 border-t border-border", children: [_jsxs("span", { className: "font-mono", children: [t('dashboard.lastUpdate'), ": ", lastUpdate] }), _jsxs("span", { children: [t('dashboard.sources'), ": CETESB \u00B7 INPE \u00B7 IBAMA \u00B7 IQAir \u00B7 DATASUS \u00B7 Open-Meteo"] })] })] })] }));
};
