...
export function getAqiBands(t: TFunction): AqiBand[] {
...
export function getUVLevels(t: TFunction): UVLevel[] {
...
export function getPollenLevels(t: TFunction): PollenLevel[] {
...
export function getPollutantInfo(t: TFunction): Record<string, PollutantInfo> {
  const result: Record<string, PollutantInfo> = {}
...
export function getAQILabel(aqi: number, t: TFunction): string {
  if (aqi <= 50)  return t('aqi.bands.good.label')
  if (aqi <= 100) return t('aqi.bands.moderate.label')
  if (aqi <= 150) return t('aqi.sensitiveShort')
...
export function getHealthAlerts(
  aqi: number,
  t: TFunction,
): Array<{ severity: 'info' | 'warning' | 'danger' | 'critical'; message: string }> {
  if (aqi <= 50) return [{ severity: 'info',     message: t('healthAlerts.good_0') }]
  if (aqi <= 100) return [{ severity: 'warning', message: t('healthAlerts.moderate_0') }]
  if (aqi <= 150) return [
