...
import { CityData, DayForecast, ForecastIcon, HealthAlert, Pollutant } from '@app-types/city.types'
...
const CITIES_SEED: Array<{ name: string; state: string; lat: number; lng: number; aqi: number }> = [
...
function getCityData(cityName: string): CityData | null {
...
export const CityDashboard = ({ cityName, onClose }: CityDashboardProps) => {
  ...
  return (
    <div className="w-80 flex-shrink-0 flex flex-col overflow-y-auto max-h-[calc(100vh-140px)] pr-1 space-y-3 animate-fade-in">
      ...
      <AQIHistoryChart history={city.history} />
