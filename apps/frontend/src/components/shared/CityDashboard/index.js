...
export const CityDashboard = ({ cityId, onClose }) => {
    ...
    if (isError || !city) {
        return (_jsxs("div", { className: "w-80 flex-shrink-0 bg-card border border-border rounded p-6 flex flex-col items-center justify-center gap-3", children: [_jsx("p", { className: "text-sm text-muted-foreground font-body text-center", children: "Dados n\u00E3o dispon\u00EDveis para esta cidade." }), _jsx("button", { onClick: onClose, className: "text-xs text-muted-foreground hover:text-foreground transition-colors", children: "Fechar" })] }));
    }
